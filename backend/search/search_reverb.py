...
def search_reverb(
    ...
    if use_mock:
        # Мок-режим: загружаем данные из файла и фильтруем по цене
        mock_data = _load_mock_data()
        return _filter_by_price(mock_data, price_min, price_max)
    ...
    if not results:
        mock_data = _load_mock_data()
        results = _filter_by_price(mock_data, price_min, price_max)
