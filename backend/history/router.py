...
async def list_sessions(
    ...
    sessions_data, total = get_sessions(user_id=current_user["id"], offset=offset, limit=limit)
...
async def new_session(
    ...
    session_id = create_session(request.title, user_id=current_user["id"])
...
async def session_messages(
    ...
    items = get_session_messages(session_id, user_id=current_user["id"])
...
async def delete_all_history(current_user: UserRecord = Depends(get_current_user)):
...
async def get_usage_stats(current_user: UserRecord = Depends(get_current_user)):
