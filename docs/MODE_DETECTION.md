...
- `conversation` — короткий conversational/meta turn без запроса на guitar advice, сравнение результатов или каталоговый поиск.
...
- persist ready-search snapshots for future router context;
...
- intent;
- conversational/meta subtypes or language preferences;
...
Those decisions belong to LLM-router and must arrive through `intent`, `enough_for_search`, `missing_fields`, `current_turn_search`, and `search_params`.

No-preference and default decisions also belong to LLM-router. If a beginner user says they cannot choose a guitar type, the router should return `no_preference_fields=["type"]` and explicit `type="any"`. Backend must not detect phrases like `не знаю` in Python. If budget is unknown, backend must not run an unlimited search: the router either requests a cheap beginner confirmation with `budget_default_offer=true`, or returns an explicit default budget in `search_params.price_max` with diagnostic `default_actions=["apply_beginner_budget"|"accept_beginner_budget"]`. `default_actions` are labels, not execution instructions.
...
For conversational/meta turns the router returns `intent="conversation"` with `search_params=null`, `missing_fields=[]`, `enough_for_search=false`, and `should_offer_search=false`. Backend maps that to external `mode="conversation"`, calls the dedicated conversation answer prompt, and does not mutate the search snapshot.
...
Backend still does not synthesize semantic fields. For ready search, the current router output must contain `current_turn_search=true` and the final effective `search_params` snapshot with 1-3 `search_queries`, an explicit price range/value, and explicit `type` or `"any"`. Backend executes that current snapshot directly, persists the same snapshot as audit/context state, and builds public `searchParams` from the same object. Stale session state cannot make a partial current route ready or silently rerun an old search.

Missing optional fields in a ready snapshot are normalized to `null` and clear old values. Contradictory ready routes, unknown `missing_fields`, invalid enum values, too many search queries, empty query lists, or `enough_for_search=true` with non-empty `missing_fields` are repairable contract errors, not silent clarification/search coercions.
...
Backend maps that to `mode="clarification"`. Clarification responses are derived from the current incomplete route/state and do not expose stale ready-search `searchParams`.
