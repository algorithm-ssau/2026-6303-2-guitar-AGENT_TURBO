...
# При необходимости поменяй LLM_ANSWER_MODEL для качества ответов.
# LLM_ROUTER_MODEL лучше держать быстрой JSON-моделью, например llama-3.1-8b-instant.
```
...
> Router и генерация ответов настраиваются отдельно: `LLM_ROUTER_MODEL` для классификации intent/search params, `LLM_ANSWER_MODEL` для консультаций/off-topic. `LLM_MODEL` остаётся fallback для answer model.
...
- В пользовательском сценарии week-07 также предусмотрены история диалогов и экспорт истории чата как часть общей UX-полировки продукта.
