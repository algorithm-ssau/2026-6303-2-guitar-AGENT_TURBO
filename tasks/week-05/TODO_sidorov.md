...
- Добавить confirm-диалог при удалении:
  - Удаление сессии → "Удалить этот чат?" → "Да" / "Отмена"
  - "Очистить всё" → "Удалить всю историю?" → "Да" / "Отмена"
  - Использовать `ConfirmDialog` компонент Фокина из `frontend/src/shared/components/ConfirmDialog.tsx` (если уже смержен) или простой `window.confirm()` как fallback
...
- Удаление сессии → подтверждение → удаление
- "Очистить всё" → подтверждение → очистка
